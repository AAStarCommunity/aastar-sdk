...
|---|---|---|---|
| 2026-06-28 | AirAccount contracts **v0.20.0 → v0.20.2** (P-256 mixed-sig module governance #127; `installModule`/`uninstallModule` relocated to AirAccountExtension — **breaking guardian-sig encoding**, see below) · SuperPaymaster **v5.4.0-beta.1-redeploy → v5.4.1-rc.1** (two-step slash guard #249 + BLS_AGGREGATOR wiring #S3; FULL Sepolia redeploy ~18 addr) · KMS **openapi 0.23.2 → 0.26.1** (server-side lowercase normalization — SDK already aligned in 0.27.1; api in-sync) · DVT **v1.5.0 → v1.6.0** (wire-format unchanged; radar 4/4) | _this PR_ | radar 4/4 in-sync; `installModule`/`uninstallModule` rewritten to v0.20.2 `abi.encode(uint8[] signerIdxs, bytes[] sigs[, bytes moduleInitData])` + `GUARDIAN_SIG_VERSION=4` digest with on-chain `moduleManagementNonce()` (#75), 13 byte-exact decode-roundtrip tests; new SP `queueSlash`/`cancelSlash`/`initBLSAggregator` wrapped (slash flow now requires `queueSlash` first). On-chain E2E acceptance pending (#209). |
