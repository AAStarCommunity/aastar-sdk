...
| 2 | Agent register/revoke via SDK | A1 register `0xeed08f22…` / revoke `0xcc46a79d…` | SDK `AgentRegistryService.encode{Register,Revoke}AgentViaAccount` produces the account-routed calldata that binds then unbinds an agent wallet (`isRegisteredAgent` true→false) | agent lifecycle managed entirely through the SDK |
...
| 5 | KMS create-account + signing | A4 KeyId `54c91d0f…` → recover ✅ | SDK `KmsManager` mints a TEE secp256k1 wallet bound to a passkey, and a WebAuthn-ceremony-gated TEE signature recovers to that wallet | passwordless TEE-custodied account + signing via the SDK |
| 6 | Session keys (secp256k1 + P256) | B1 grant/revoke `0xe5f16c4f…`/`0xaa2885a6…` | SDK `SessionKeyService` grants then revokes both a secp256k1 and a P256 session; `isSessionActive` flips true→false | delegated, scoped, revocable session keys via the SDK |
