...
| ~~G6~~ ✅ **DONE** | **CC-13 批A+批B 均已发布** —— 批A read getter(`slashPolicyAdmin`/`slashThresholds`)@ v0.39.2；批B Timelock 写编排(`timelockControllerActions` schedule/execute/hashOperation/getTimestamp/getMinDelay + `encodeSetSlashPolicyAdmin`/`encodeSetSlashThreshold` calldata)@ **v0.39.3**（PR #297，含 #297 Low 的 salt 复用告警修复）。TimelockController ABI 已 vendored。 | [测] | — |
...
| G9(部分✅) | **#176 Tier2/3 API** —— (1)判档 + fail-fast + 签名 needs **早已由 `resolveTransfer`(core/tier) 统一覆盖**(ETH+ERC20，返回 `{tier, requiredSigs, limits, blockReason}`，含 strictMode 拦截未配置 token)。(3)限额查询 `getGuardState`(ETH) done；本轮补齐 **ERC20 per-token 限额读** `getTokenGuardState`(从 stub→真读 `tokenConfigs`)+ 修两个 pre-existing bug:① tier 边界 `<`→`<=`(对齐链上 `requiredTier`)；② **token tier 语义**——新增 `resolveTokenTier` 镜像链上 GUARD `recordTokenSpend`(`tier2Limit==0` = 无上限 T2,而非 account `requiredTier` 的 T3),`getTokenGuardState` + `resolveTransfer` 的 token 路径都改用它(此前误用 account 语义 `resolveTier`)。**剩 (2) 转账侧 Tier-3 guardian ECDSA 收集**(prepareTransfer/submitPreparedTransfer 目前 WebAuthn-only)—— 较大，需 guardian 账户链上 E2E，单列后续。 | [测]已交付 / [主]part2 | SDK 自查(part2 需链上 E2E) |
