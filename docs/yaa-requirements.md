...
- **Solution (confirmed 2026-06-14):** (a) add on-chain registry query wrappers for what IS readable (`getCommunityByName/ENS`, role config, stake, membership); (b) **rich profile via event→calldata back-trace (free, own RPC — the standard indexer pattern):** the `RoleRegistered(roleId, user, burnAmount, timestamp)` event does NOT carry roleData, but the profile WAS submitted in the `registerRole(roleId, user, roleData)` **transaction calldata**. So `getCommunityProfile(community)` = `eth_getLogs(RoleRegistered, [ROLE_COMMUNITY, community])` → fetch that tx → decode its `registerRole` calldata → extract+decode `roleData` (name/ensName/website/description/logoURI). Zero cost, no paid indexer. Scale-out option: a The Graph subgraph (free tier) or a cached index — but the base path is pure RPC. (NOT a contract change; works on the already-slimmed Registry.)
