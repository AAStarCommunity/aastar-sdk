...
export const AIRACCOUNT_ADDRESSES = {
  sepolia: {
    ...
    // and these fields re-derive automatically (currently AirAccount v0.20.3, Sepolia
    // 2026-06-28 — gasless self-call patch #140; impl/extension/factory/agentRegistry
    // redeployed). The key mapping (airaccount field ← core key) is asserted by
    // entrypoint.addresses.test.ts so it can't silently drift.
...
export const AIRACCOUNT_ABI = [
  ...
  // Tier thresholds — standalone getters on the ACCOUNT (uint256). v0.22.0 REMOVED getConfigDescription()
  // (which used to carry these in its struct); read the getters directly. guard-checker / prepareTransfer
  // depend on these on every transfer — a missing declaration reverts the whole tiering path (#254 class).
