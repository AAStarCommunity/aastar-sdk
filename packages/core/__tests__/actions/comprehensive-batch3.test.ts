...
describe('Comprehensive Action Tests Batch 3', () => {
  ...
  describe('Aggregator Extended', () => {
    ...
    it('executeProposal', async () => { w.writeContract.mockResolvedValue('0x' as `0x${string}`); await aggregatorActions(A)(w).executeProposal({ proposalId: '0x01', target: U, call Data: '0x', requiredThreshold: 2n, proof: '0x', account: w.account }); expect(w.writeContract).toHaveBeenCalled(); });
