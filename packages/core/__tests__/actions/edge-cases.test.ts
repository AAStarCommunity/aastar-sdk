...
describe('Edge Cases & Error Handling Tests', () => {
  ...
  describe('Registry Edge Cases', () => {
    ...
    it('unregister non-existent role', async () => { w.writeContract.mockResolvedValue('0x' as `0x${string}`); await registryActions(T)(w).unRegisterRole({ roleId: 999n, account: w.account }); expect(w.writeContract).toHaveBeenCalled(); });
  ...
  describe('Staking Edge Cases', () => {
    ...
    it('getStakeInfo returns empty', async () => { p.readContract.mockResolvedValue({ amount: 0n, timestamp: 0n }); const r = await stakingActions(T)(p).getStakeInfo({ user: U, roleId: 1n }); expect(r).toBeDefined(); });
  ...
  describe('Paymaster Edge Cases', () => {
    it('depositFor with zero amount', async () => { w.writeContract.mockResolvedValue('0x' as `0x${string}`); await paymasterActions(T)(w).depositFor({ account: U, amount: 0n, txAccount: w.account }); expect(w.writeContract).toHaveBeenCalled(); });
    it('withdraw entire balance', async () => { w.writeContract.mockResolvedValue('0x' as `0x${string}`); await paymasterActions(T)(w).withdraw({ amount: 999999n, account: w.account }); expect(w.writeContract).toHaveBeenCalled(); });
    it('balances returns zero', async () => { p.readContract.mockResolvedValue(0n); expect(await paymasterActions(T)(p).balances({ account: U })).toBe(0n); });
  ...
  describe('Error Conditions', () => {
    ...
    it('handles invalid address format', async () => { w.writeContract.mockResolvedValue('0x' as `0x${string}`); await tokenActions()(w).transfer({ token: T, to: '0xinvalid' as any, amount: 1n, account: w.account }); expect(w.writeContract).toHaveBeenCalled(); });
  });
