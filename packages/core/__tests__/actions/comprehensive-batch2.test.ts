...
describe('Comprehensive Action Tests Batch 2', () => {
  ...
  describe('SuperPaymaster Extended', () => {
    it('operators', async () => { p.readContract.mockResolvedValue([0n, true, false, U, 0, 0, U, 0n, 0n]); const res = await superPaymasterActions(A)(p).operators({ operator: U }); expect(res.isConfigured).toBe(true); });
