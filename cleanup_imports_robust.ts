...
const STANDARD_IMPORTS = `import { createPublicClient, createWalletClient, http, formatEther, parseAbi, keccak256, toHex, type Hex, parseUnits } from 'viem';
...
SCRIPTS.forEach(scriptPath => {
    ...
    const filteredLines = lines.filter(line => {
        ...
        if (trimmed.startsWith('import {') && (trimmed.includes('viem') || trimmed.includes('module'))) return false;
        if (trimmed.startsWith('import * as') && (trimmed.includes('dotenv') || trimmed.includes('path'))) return false;
        if (trimmed.startsWith('import dotenv')) return false;
        if (trimmed.startsWith('import path')) return false;
        ...
        if (trimmed.startsWith('const require =')) return false;
        if (trimmed.startsWith('type Hex =')) return false;
        if (trimmed.includes('BigInt.prototype as any).toJSON')) return false;
        if (trimmed === '}') return false; // Match the stray brace from previous failed cleanup
        return true;
    });

    const newContent = STANDARD_IMPORTS + '\n' + filteredLines.join('\n');
    const finalContent = newContent.replace(/\\n\\n\\n+/g, "\\n\\n");
    fs.writeFileSync(scriptPath, finalContent);
    console.log(`✅ Robustly Standardized ${scriptPath}`);
});
