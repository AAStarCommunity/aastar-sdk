...
const ACCOUNT_ADDRESS = (process.env.TEST_SIMPLE_ACCOUNT_B) as Address;
const RECEIVER = "0x93E67dbB7B2431dE61a9F6c7E488e7F0E2eD2B3e";

async function main() {
    console.log(`🚀 Group B: Paymaster V4 (AOA Mode) (Network: ${NETWORK})`);
    
    if (!SIGNER_KEY) throw new Error("Missing Private Key (PRIVATE_KEY_JASON)");
    if (!ACCOUNT_ADDRESS) throw new Error("Missing TEST_SIMPLE_ACCOUNT_B");
    ...
    try {
        // Note: The SDK's executeGasless will automatically try to find a Paymaster.
        // For V4 / AOA mode, we ensure the SDK is configured to target the V4 Paymaster.
        // If not specified, it probes the Registry.
        
        console.log("   ⚡ Executing Gasless Transfer (Paid by V4 Paymaster)...");
        ...
        console.error("   ❌ V4 Test Failed:", error.message.split('\n')[0]);
    }
}
